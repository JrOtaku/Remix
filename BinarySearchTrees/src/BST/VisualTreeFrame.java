...
public class VisualTreeFrame extends JFrame {
	static BinarySearchTree<Integer> b = null;
...
//		b.insert(3);
	...
	class Panel extends JPanel {
		...
		public void paintComponent(Graphics comp) {
			...
			if (b.root != null) {
				BinarySearchTree.BinaryNode root = b.root;
				Node n = new Node(this.getWidth() / 2 - 12.5, 25,
						(Integer) root.getElement());//getElement()
				n.drawOn(comp2D);
		...
		public void drawRec(Node n, BinarySearchTree.BinaryNode par,
			...
			if (par.getLeftChild() != null) {
				Line2D l1 = new Line2D.Double(x+12.5, y +25, x - tx + 12.5, y
						+ ty);
				comp2D.draw(l1);
				BinarySearchTree.BinaryNode apar = par.getLeftChild();
				n.translate(x - TRANSX, y + TRANSY, (Integer) apar.getElement()); // getElement
				n.drawOn(comp2D);
			...
			if (par.getRightChild() != null) {
				Line2D l1 = new Line2D.Double(x + 12.5, y +25, x + tx+12.5, y
						+ ty);
				comp2D.draw(l1);
				BinarySearchTree.BinaryNode bpar = par.getRightChild();
				n.translate(x + TRANSX, y + TRANSY, (Integer) bpar.getElement()); // getElement
				n.drawOn(comp2D);
		...
		class Node extends JComponent {
			...
			public void translate(double x, double y, int v) {
				this.CenterX = x;
				this.CenterY = y;
				this.value = v;
				}
